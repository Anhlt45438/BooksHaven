...
class storyServices {
  ...
  async getAllRating(story_id: string, page: number, limit: number, user_id: string ) {
    try {
      const data = await databaseServices.ratings
        .find({
          story_id: new ObjectId(story_id),
          user_id: new ObjectId(user_id)
        })
        // .project({ _id: 1, points: 1, index: 1,user_name:1 })
        .sort({ index: 1 })
        .limit(limit)
        .skip(limit * page)
        .toArray();
